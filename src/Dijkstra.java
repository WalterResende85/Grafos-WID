...
public class Dijkstra {
    ...
    public ArrayList<Vertice> encontrarMenorCaminhoDijkstra(Grafo grafo, Vertice v1,
            Vertice v2) {
        ...
        for (int i = 0; i < grafo.getGrafo().size(); i++) {
            ...
            // 9999("infinita")
            if (grafo.getGrafo().get(i).getNome()
                    .equals(v1.getNome())) {
        ...
        while (!this.naoVisitados.isEmpty()) {
             ...
             */
            for (int i = 0; i < atual.getListaArestaSai().size(); i++) {
                ...
                if (!vizinho.verificarVisita()) {
                    ...
                    if (vizinho.getDistancia() > (atual.getDistancia() + atual
                            .getListaArestaSai().get(i).getPeso())) {

                        vizinho.setDistancia(atual.getDistancia()
                                + atual.getListaArestaSai().get(i).getPeso());
        ...
        for (int i = 0,j = 1; j < menorCaminho.size(); i++,j++) {
            menorCaminho.get(i).setMenorCaminho(menorCaminho.get(i).menorCaminhoParaX(menorCaminho.get(j)));
        }
